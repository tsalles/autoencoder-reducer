...
for i in `seq 0 $((num_folds-1))`; do
  echo "Round $i"
  python3 model.py -t $dataset/${num_folds}fold/train$i -T $dataset/${num_folds}fold/test$i -o $dataset/preds_bl.out -c 512 -S 64 -e 50 -v 0.3 -r $i -p $dataset/hist_bl$i;
  python3 model.py -t $dataset/${num_folds}fold/train$i -T $dataset/${num_folds}fold/test$i -a 1024 512 -b 256  -c 512 --autoencoder --pretrain-ae -E 50 -e 50 -v 0.3 -r $i -p $dataset/hist$i -B $dataset/reduced$i -o $dataset/preds_ae.out -s $dataset/ae_model$i -S 64;
  python3 model_layered.py -t $dataset/${num_folds}fold/train$i -T $dataset/${num_folds}fold/test$i -a 1024 512 -a 256 128 -a 64 32 -b 128 64 32 -c 512  --autoencoder --pretrain-ae -E 50 -e 50 -v 0.3 -r $i -p $dataset/hist_layered$i -B $dataset/reduced_layered$i -o $dataset/preds_ae_layered.out -s $dataset/ae_model_layered$i -S 64 -B $dataset/reduced_layered$i;
