...
class VariationalAutoEncoder(tf.keras.layers.Layer):
    def __init__(self, input_dim, bottleneck_dim, ae_dims, activation='relu'):
        ...
        self.encoders = [tf.keras.layers.Dense(input_dim, activation='linear')]
        self.encoders += [tf.keras.layers.Dense(dim, activation=activation) for dim in ae_dims]

        self.dense_mean = tf.keras.layers.Dense(bottleneck_dim)
        self.dense_log_var = tf.keras.layers.Dense(bottleneck_dim, kernel_initializer=tf.keras.initializers.Zeros())
...
#                                                kernel_regularizer=tf.keras.regularizers.OrthogonalRegularizer(factor=0.01, mode="columns"))
    ...
    def call(self, input_tensor, training=False):
        ...
        reduced = self.sampling((z_mean, z_log_var))
        x = reduced
...
class HVAE(tf.keras.Model):
    def __init__(self, input_dim, hae_dims, bottleneck_dims, num_classes, ae_activation='relu', supervised=True):
        ...
        for level, ae_dims in enumerate(hae_dims):
            self.autoencoders.append(VariationalAutoEncoder(input_dim, bottleneck_dims[level], ae_dims, ae_activation))
            if supervised:
    ...
    def call(self, input_tensor, training=False, **kwargs):
        ...
        pairs = []
        for level, vae in enumerate(self.autoencoders):
            reconstructed, reduced = vae(current_input)
...
def build_model(input_dim, batch_size, hae_dims, bottleneck_dims, num_classes):
    ...
    hae = HVAE(input_dim, hae_dims, bottleneck_dims, num_classes)
...
#  tf.keras.utils.plot_model(hae.build_graph((x_trn.shape[-1])), to_file="model.png", expand_nested=True, show_shapes=True)
  ...
  predict(tst_dataset, hae, alpha, args.output, args.bottleneck_output if args.bottleneck_output else args.output+'.reduced')
